...
function getPreferredLocale(request: NextRequest): string {
  ...
  const preferred = acceptLanguage.split(",").map((lang) => lang.split(";")[0].trim().substring(0, 2)).find((lang) => isValidLocale(lang));
...
export function middleware(request: NextRequest) {
  ...
  return NextResponse.redirect(new URL(`/${locale}${pathname}`, request.url), 302);
}
