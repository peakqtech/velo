...
export function EmberFooter({ content, localeSwitcher }: EmberFooterProps) {
  ...
  return (
    <footer className="ember-footer-section relative bg-secondary" aria-label="Site footer">
      <div className="w-12 h-[1px] bg-primary mx-auto" />
      <div className="ember-footer-inner py-section px-6">
        <div className="max-w-content mx-auto pt-12">
          <motion.div className="text-center mb-20"
            ...
            {submitted ? (
              <motion.div className="flex items-center justify-center gap-2 text-primary font-medium mt-6"
                initial={shouldReduceMotion ? {} : { opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <form onSubmit={handleSubmit} className="flex max-w-md mx-auto gap-2 mt-6">
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={newsletter.placeholder} required aria-label="Email address"
                  className="flex-1 px-5 py-3 bg-background border border-foreground/10 text-foreground placeholder:text-muted focus:outline-none focus:border-primary transition-all duration-300" />
                <button type="submit" className="px-6 py-3 bg-primary text-white font-display tracking-wider hover:bg-primary-light transition-all duration-300">{newsletter.cta}</button>
              </form>
            )}
          </motion.div>
          ...
          <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-16"
            ...
            {links.map((group) => (
              <motion.div key={group.group} variants={fadeInUp}>
                <h3 className="font-display font-bold text-foreground mb-4">{group.group}</h3>
                <ul className="space-y-3">
                  {group.items.map((item) => (<li key={item.label}><a href={item.href} className="text-muted hover:text-foreground transition-colors text-sm">{item.label}</a></li>))}
                </ul>
              </motion.div>
            ))}
          </motion.div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-foreground/10">
            <div className="flex items-center gap-4"><p className="text-sm text-muted">{legal}</p>{localeSwitcher}</div>
            <div className="flex gap-3 mt-4 md:mt-0">
              {socials.map((social) => (
                <a key={social.platform} href={social.url} target="_blank" rel="noopener noreferrer" aria-label={social.platform}
                  className="p-2.5 rounded-full border border-foreground/10 text-muted hover:text-foreground hover:border-primary/30 transition-all duration-300 text-sm">{social.platform}</a>
              ))}
            </div>
          </div>
        </div>
      ...
      <motion.button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Back to top"
        className="fixed bottom-8 right-8 z-40 p-3 rounded-full bg-primary text-white hover:bg-primary-light transition-all duration-300"
